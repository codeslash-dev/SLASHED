...
function sweepFamily([family, steps, sourceLs]) {
  ...
  const sweeps = sourceLs.map(L => {
    root.style.setProperty(`--sf-color-${family}-source-light`, `oklch(${L} 0.16 264)`);
  ...
  el.remove();
  root.style.removeProperty(`--sf-color-${family}-source-light`);
  return sweeps;
}

test.describe('Brand ramp is monotonic for any source colour', () => {
  for (const family of FAMILIES) {
    test(`${family} ramp never folds across the lightness axis`, async ({ page }) => {
      await page.goto(FIXTURE);
      const sweeps = await page.evaluate(sweepFamily, [family, STEPS, SOURCE_LS]);
      ...
      for (const { L, rows } of sweeps) {
        for (const { step, alpha } of rows) {
          expect(alpha, `${family}-${step} (source L=${L}) should be opaque`).toBe(255);
        }
        for (let i = 0; i < rows.length - 1; i++) {
          expect(
            rows[i + 1].lum,
            `${family} (source L=${L}): step ${rows[i + 1].step} must not be lighter than ${rows[i].step}`
