...
- Quick-start eksponuje 15 ręcznych `<link>` w wymuszonej kolejności zamiast pre-built bundla, więc każdy `<link>` wstawiony przed `core/layers.css` cicho łamie kaskadę u konsumenta (F-07).
...
- 106 wyrażeń `oklch(from …)` w `core/tokens.css` i ≈126 wywołań `color-mix()` napędzających 132 wyprowadzone tokeny w `optional/tokens.palette.css` są przeliczane per-repaint, co spowalnia theme-toggle nieudokumentowanym kosztem (F-12).
...
Severity:   high
Category:   docs
Evidence:   `README.md:117-120` deklaruje cascade-layer baseline jako próg:
```text
...
### F-07 — README promuje 15 ręcznych `<link>` ponad pre-built bundle, co czyni kolejność load-bearing
Severity:   medium
Category:   foot-gun
Evidence:   `README.md:17-36` przedstawia jako pierwsze 15 niezależnych `<link>` w ścisłym porządku (`core/layers.css` najpierw, `optional/legacy.css` ostatni); dopiero `README.md:38-47` opcjonalnie ("**Recommended:**") wspomina o `slashed.essential.css` / `slashed.full.css`:
```html
...
```
Tokeny `--sf-border-width-1` (`core/tokens.css:464`) i `--sf-border-width-2` (`core/tokens.css:465`) już istnieją w sekcji "Borders" — wystarczy je skonsumować zamiast ponownie wpisywać literały.
Effort:     XS

### F-09 — `core/accessibility.css` używa literału `44px` zamiast tokena `--sf-touch-target`
Severity:   low
Category:   inconsistency
Evidence:   `core/accessibility.css:94-97`:
```css
a {
  min-height: 44px;
  min-width:  44px;
...
Impact:     Strona z dużą paletą (`slashed.full.css` ładuje też `optional/tokens.palette.css` z dodatkowymi ≈126 wywołaniami `color-mix()` napędzającymi 132 wyprowadzone tokeny) zmusza silnik renderujący do re-rezolwowania wszystkich relative-color expressions przy każdej zmianie `--sf-color-primary` (np. theme-toggle). Na słabszym sprzęcie mobilnym pojedynczy theme-switch jest mierzalnie wolniejszy niż w Pico/Tailwind. Brak dokumentacji tej charakterystyki.
...
Severity:   low
Category:   perf
Evidence:   `grep -c 'color-mix' optional/tokens.palette.css` zwraca `127` (1 linijka komentarza w nagłówku pliku + 126 faktycznych wywołań w deklaracjach tokenów). Próbka z `optional/tokens.palette.css:30-45`:
```css
...
```
6 brand × (11 stopni numerycznych + 11 wariantów alfa) = 132 wyprowadzone tokeny; 6 z nich (`*-500` per brand) to aliasy `var()` bez własnego `color-mix`, więc realnych wywołań jest 126 — wszystkie eager-evaluated.
...
Impact:     `slashed.full.css` ma dodatkowy koszt repaintu wprost proporcjonalny do liczby wyliczanych tokenów (≈126 wywołań `color-mix()` na 132 derived tokenach). Na stronach lazy-load sekcjach z `data-theme="dark"` widoczny jest paint-tearing przy pierwszym wejściu w dark section. Konsument bez wiedzy o tym zjawisku diagnozuje to jako "framework jest wolny".
Recommendation: Pozostawić architekturę — to świadomy tradeoff za no-build. Rozszerzyć README § Bundles o akapit: "`slashed.full.css` adds 132 derived palette tokens computed via ≈126 `color-mix(in oklch …)` calls. If you only need brand colors, link `slashed.essential.css` instead — palette is opt-in." Plus w `docs/architecture.md` § Tokens: "Palette is opt-in; cost-neutral if you only ship `slashed.essential.css`."
Effort:     XS
...
Impact:     Czytelnik README wykonuje quick-start, dodaje 15 tagów, potem widzi 0 efektu od `components.css`/`utilities.css` i traci czas na diagnostykę "czemu nie ma buttona". Trzy `<link>` to 3 zbędne HTTP request bez korzyści przez cały okres do uzupełnienia warstwy komponentów.
...
✓ confirmed — WARN-2 (`sign(0.6 - l)` produces 0 when l = exactly 0.6). Wzór nadal jest binarny, dyskontynuacja przy L=0.6 niezmieniona. `core/tokens.css:152-163` używa `sign(0.6 - l) * 999` na dziesięciu tokenach `text--on-*` (linie 152-156 i 159-163; 157-158 to aliasy `var()` bez `sign()`). To intencjonalny tradeoff z `docs/architecture.md:163-170` (patrz polemika w sekcji 7).

✓ confirmed — WARN-3 (`tertiary` and `neutral` text-on-color only pass AA Large in light mode). `docs/architecture.md:163-170` § Known intentional tradeoffs eksplicytnie kodyfikuje "AA Large, not AA Normal" dla `tertiary`/`neutral`; źródłowe tokeny w `core/tokens.css:154, 156` oraz default `core/tokens.css:52, 54` (L≈0.55) potwierdzają liczbowo regułę.
...
| 2 | Wprowadzić bundle-first quick-start w README (jedna linia `<link>`) i zwinąć 15-tagowy setup w `<details>` z notą "Order matters" | F-07 | XS | Aktualna kolejność prowadzi konsumenta przez foot-gun, w którym pierwszy zły `<link>` cicho łamie kaskadę. |
...
| # | Item | F-NN | Effort | Rationale (1 sentence) |
| --- | --- | --- | --- | --- |
| 1 | Zamienić literały `1px solid #999`, `2px solid` i `2px dashed` w `core/print.css` i `core/states.css` na istniejące tokeny `--sf-border-width-1`/`--sf-border-width-2`/`--sf-color-border*` | F-08 | XS | Trzy literały obchodzą zasadę "every value via var()" z `docs/architecture.md` i blokują rebrand-skinning. |
...
`docs/architecture.md:163-170` § Known intentional tradeoffs kodyfikuje zachowanie tekstu na kolorowym tle dosłownym cytatem:
...
Implementacja siedzi w `core/tokens.css:152-163` i powtarza wyrażenie 10 razy (linie 152-156 i 159-163; 157-158 to aliasy `var()`):
...
Argument (severity `medium`, kategoria `a11y`): twierdzenie "binary threshold cannot remove it" jest prawdziwe wyłącznie w obrębie samego operatora `sign()` — nie w obrębie języka, w którym jesteśmy. Architektura akceptuje skokową dyskontynuację o 0.85 jednostki perceptual lightness w mikro-otoczeniu L=0.6, mimo że identyczna decyzja "biały albo czarny" daje się zapisać jako funkcja ciągła z dowolnie wąską ramką przejściową. To znaczy: konsument SLASHED, który zmieni `--sf-color-tertiary-light` z `oklch(0.55 0.14 310)` na `oklch(0.61 0.14 310)`, zaobserwuje natychmiastowy flip z białego tekstu na czarny przy zmianie o 0.06 jednostki perceptual lightness — bez żadnej mediany. Gradient `clamp(0.1, sign(0.6 - l) * 999, 0.95)` jest stopnia 0; każda funkcja stopnia 1 znika.

Konstruktywna alternatywa, wciąż CSS-only (bez Sass / PostCSS / JS), dla `core/tokens.css:152-163`:

```css
/* Wąskie pasmo przejściowe ~0.5775 ≤ l ≤ 0.6225 zamiast skoku przy l = 0.6 */
--sf-color-text--on-tertiary:  oklch(from var(--sf-color-tertiary)   clamp(0.1, calc((0.6 - l) * 20 + 0.5), 0.95) 0 0);
--sf-color-text--on-neutral:   oklch(from var(--sf-color-neutral)    clamp(0.1, calc((0.6 - l) * 20 + 0.5), 0.95) 0 0);
```

Wyrażenie `clamp(0.1, calc((0.6 - l) * 20 + 0.5), 0.95)` przy nachyleniu `* 20` zwija ramkę przejściową do ≈0.045 jednostki l wokół progu, więc dla wszystkich domyślnych palet SLASHED tekst pozostaje praktycznie tak jasny lub tak ciemny jak przy formule binarnej:
- przy `l = 0.40` zwraca `4.5` → `clamp` ścina do `0.95` (biały tekst, ΔL=0.55 — identycznie jak dziś),
- przy `l = 0.50` zwraca `2.5` → `clamp` ścina do `0.95` (biały tekst, ΔL=0.45 — identycznie jak dziś),
- przy `l = 0.55` (default `tertiary`/`neutral`) zwraca `1.5` → `clamp` ścina do `0.95` (biały tekst, ΔL=0.40 — **identycznie jak formuła binarna**, bez regresji w korycie),
- przy `l = 0.5775` zwraca `0.95` (graniczny biały — początek ramki przejściowej),
- przy `l = 0.60` zwraca `0.5` (środkowy szary, jedyny punkt globalnego minimum kontrastu — tekst L=0.5 vs tło L=0.6, ΔL=0.10),
- przy `l = 0.6225` zwraca `0.05` → `clamp` podnosi do `0.1` (graniczny czarny — koniec ramki przejściowej),
- przy `l = 0.70` zwraca `-1.5` → `clamp` podnosi do `0.1` (czarny tekst, ΔL=0.60 — identycznie jak dziś),
- przy `l = 0.80` zwraca `-3.5` → `clamp` podnosi do `0.1` (czarny tekst, ΔL=0.70 — identycznie jak dziś).

Cała funkcja jest `clamp(min, calc(…), max)` — standardowy CSS Values Level 4, dostępny w tym samym oknie wsparcia co `sign()` (cross-browser dopiero od mid-2024; precyzyjne numery wersji w F-02). Brak nowych tokenów `@property`, brak nowego at-rule, brak narzędzi build. Polemika nie polega na tym, że nowa formuła „dodaje" kontrast — każda ciągła interpolacja między czarnym a białym po pasmie l musi gdzieś osiągnąć minimum kontrastu wewnątrz tego pasma, więc punkt L≈0.6 staje się lokalnym dołkiem zamiast krawędzi skoku. Polemika polega na tym, że ten dołek można uczynić **arbitralnie wąskim** (slope 20 → ramka ≈0.045; slope 50 → ramka ≈0.018) zachowując przy tym różniczkowalność w l=0.6. Konsekwencja: konsument zmieniający `--sf-color-tertiary-light` o 0.06 jednostki l otrzymuje płynne przejście zamiast natychmiastowego flipa, ale dla `tertiary`/`neutral` (L=0.55) wartość kontrastu jest **niezmieniona** względem dzisiejszego `~4.2:1`. Cena: hue, których l mieści się dokładnie w paśmie 0.5775–0.6225 (np. `action` L=0.60 albo użytkownicki brand z L=0.59), wpada w lokalne minimum kontrastu — ale to ten sam pas wartości, który dziś leży po jednej albo drugiej stronie skoku z efektem "AA-Large w light, AA-Large w dark" zależnie od kierunku zaokrąglenia.

Polemika jest projektowa: **gradualny dołek wokół L=0.6 jest perceptualnie preferowalny względem skokowej krawędzi**, bo flip widoczny przy zmianie tła o 0.001 l (np. theme-toggle z `light-dark()`) jest uciążliwszy wizualnie niż chwilowy spadek do ΔL=0.10 w przeglądowej ramce 0.045 jednostki l. Dla całego pasma `0.5775 < l < 0.6225` długoterminową odpowiedzią jest natywne `contrast-color()`, dziś jeszcze niedojrzałe — slope-20 to most.

Rekomendacja: rozważyć przepisanie 10 wierszy `core/tokens.css:152-163` na powyższą formułę i zaktualizowanie `docs/architecture.md` § Known intentional tradeoffs o zdanie "softened threshold collapses the discontinuity into a ≈0.045-wide transition band around L=0.6" zamiast obecnego "binary decision … cannot remove it". To NIE jest defekt do natychmiastowego naprawienia — to polemika z wyborem architektonicznym, dla którego istnieje czysto-CSS-owe ulepszenie zachowujące dotychczasowe gwarancje kontrastu poza wąskim pasmem progowym.
...
| 7 | Section 7 contains ≥ 1 polemic with an authorial tradeoff | pass | Sekcja 7.2 "Polemika 1" cytuje verbatim `docs/architecture.md:163-170` § Known intentional tradeoffs, kontestuje binarny `sign(0.6 - l)` z `core/tokens.css:152-163` i proponuje konkretną CSS-only alternatywę `clamp(0.1, calc((0.6 - l) * 20 + 0.5), 0.95)` respektującą no-build constraint. `awk '/^## 7\./,/^## 8\./' \| grep -cE 'docs/architecture\.md'` = **3**. |
