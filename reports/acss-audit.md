...
|---|---|---|
| Model podstawowy | **BEM-first (po pivocie 4.0)** — mocno **zredukowane** utilities, nacisk na zmienne + recipes + mixiny. Nadal jest zestaw utilities (spacing, display, text itd.), ale sporo klas usunięto. | **Token-first / BEM-first** — tokeny `--sf-*` + prymitywy layoutu + makra. `optional/utilities.css` jest już **wypełniony warstwą utility (hover, `.sf-h1..-h6`, z-index, object-fit, text-wrap), ale wciąż ZASZTELOWANY (zakomentowany)** — w wyjściu SLASHED nadal **nie emituje klas narzędziowych w 0.x** (czekają na odkomentowanie). |
...
- **SLASHED:** ✅ **Mamy (od PR #519) — `.sf-corner-scoop`.** Wklęsłe narożniki realizowane **nowocześnie przez `mask: radial-gradient`** (bez shadow-hacku ACSS), warianty `--top-left/-top-right/-bottom-left/-bottom-right`, sterowane `--sf-corner-scoop-at/-size`. Live w `core/macros.css`. Komentarz w kodzie wskazuje ścieżkę migracji do natywnego `corner-shape: scoop`.
- **Różnica:** Nasza metoda `mask` **działa też na gradientach/wzorach** (czego shadow-hack ACSS nie potrafi) i nie ma artefaktu „eyelash". ACSS pozwala na do 2 narożników z 16 pozycjami „na zewnątrz"; nasze jest prostsze (4 narożniki karty). Dla typowych „scoop" — parytet/lepiej.
...
- **Zrobione (PR #516):** dodano prymityw **`.sf-grid-flex`** (flex-wrap + `--sf-grid-min`, warianty `--xs…--2xl`) oraz **`.sf-grid-flex--center`** — wyśrodkowanie niepełnego wiersza, **breakpoint-free**. To jest wprost odpowiednik „centered unbalanced flex grid" bez media queries. Luka zamknięta → ✅.
...
| `?overlap` / `?overlap-alt` | tło „zachodzące" na sąsiada (angled dividers) | **`.sf-overlap` / `--down/--start/--end` + `.sf-overlap-host`** (PR #519) | ✅ (dostarczone) |
