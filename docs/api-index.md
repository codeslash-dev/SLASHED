...
| `--sf-size-l` | PUBLIC | knob | size | `2.75rem` | Large UI component height (~44px at the default size scale). A configurable design-scale token — not an accessibility guarantee. The WCAG 2.5.5 touch-target floor is enforced independently by --sf-touch-target. |
...
| `--sf-touch-target` | PUBLIC | consumption | touch | `max(44px, var(--sf-size-l))` | Minimum touch-target size for interactive elements per WCAG 2.5.5. An accessibility floor, not a design-scale token: max()-clamped to at least 44px so retuning the configurable UI size scale smaller can never drop it below the WCAG minimum. |
