...
// ── llms.txt ─────────────────────────────────────────────────────────────────
// Fully generated by scripts/gen-llms-txt.js (npm run docs); version-sync only
// re-stamps the `# SLASHED vX.Y.Z` header so the release `sync-main` job, which
// runs version-sync but not a full docs rebuild, keeps main's copy current.
